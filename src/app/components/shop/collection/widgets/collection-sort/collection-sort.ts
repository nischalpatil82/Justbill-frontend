...
@Component({
  ...
  imports: [CommonModule, Select2Module, TranslateModule, Sidebar],
...
export class CollectionSort {
  ...
  @Input() filter: Params;
  readonly products = input<number>();
  readonly topFilter = input<boolean>();
  readonly sidebarPopup = input<boolean>();
  readonly gridCol = input<string>();

  readonly setGridClass = output<{
    class: string;
    list_view: boolean;
  }>();
  ...
  constructor() {
    ...
    if (this.isBrowser) this.width = window.innerWidth;
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (gridCol) {
      ...
      if (this.gridArray.includes(layout)) {
        ...
        layout == 'collection_top_filter' ||
        layout == 'collection_sidebar_popup'
      ) {
