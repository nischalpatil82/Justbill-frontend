...
import { SizeChartModal } from '../../../modal/size-chart-modal/size-chart-modal';

@Component({
  selector: 'app-product-box-variant-attributes',
  imports: [CommonModule, CurrencySymbolPipe, TranslateModule, Button, NgbTooltip],
...
export class ProductBoxVariantAttributes {
  ...
  readonly isAllVariantStyleDropdown = input<boolean>(false);
  readonly owlCar = input<CarouselComponent>();
  readonly showPrice = input<boolean>();
  readonly showVariableType = input<string[]>([
    'color',
    'rectangle',
    'circle',
    'radio',
  ...
  readonly selectVariation = output<IVariation>();
  ...
  public hoverVariation: IVariation | null;
  ...
  attribute_value.variation_image = variation.variation_image ?? undefined;
  ...
  setVariant(variations: IVariation[], value: IAttributeValue, event?: string) {
    ...
    variations?.forEach(variation => {
      ...
      if (doValuesMatch) {
        ...
        productValue['sku'] = this.selectedVariation
          ? this.selectedVariation?.sku
          : productValue?.sku;
        productValue['sale_price'] = this.selectedVariation
          ? this.selectedVariation?.sale_price
          : productValue?.sale_price;
  ...
  openSizeChartModal(image: IAttachment) {
